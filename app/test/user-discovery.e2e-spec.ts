...
import { UserProfile, Gender, GenderPreference } from "../src/microservices/user/entities/user-profile.entity";
...
describe("UserDiscoveryController (e2e)", () => {
  ...
  async function seedTestUser(userData: Partial<User>, profileData: Partial<UserProfile>): Promise<{ user: User, profile: UserProfile }> {
  ...
  async function seedBlockRelationship(requesterId: string, recipientId: string): Promise<UserRelationship> {
  ...
  beforeAll(async () => {
    ...
    userRelationshipRepository = moduleFixture.get<Repository<UserRelationship>>(getRepositoryToken(UserRelationship));

    // 4. Seed Initial Data using helper
    const { user, profile } = await seedTestUser(
        { email: "e2e-user@test.com", username: "e2e_user", displayName: "E2E Test User" },
        { gender: Gender.MALE, birthDate: new Date("1990-01-01"), genderPreference: GenderPreference.BOTH, lastActiveAt: new Date() }
    );
    testUser = user;
    testUserProfile = profile; // Store profile as well

    // 5. Generate JWT for the test user
    jwtToken = await generateTestJwt({ userId: testUser.id, email: testUser.email });
  });
  ...
  describe("GET /api/users/discovery/homepage", () => {
    ...
    it("should return 200 OK with valid recommendations array", async () => {
      ...
      const { user: candidateUser, profile: candidateProfile } = await seedTestUser(
        { displayName: "Candidate A" },
        { gender: Gender.FEMALE, birthDate: new Date("1992-05-10"), lastActiveAt: new Date() }
      );
      ...
      const foundCandidate = response.body.find((rec: any) => rec.id === candidateUser.id);
    ...
    it("should exclude users blocked by the current user", async () => {
       ...
       const { user: candidateUser } = await seedTestUser({}, { lastActiveAt: new Date() });
    ...
    it("should filter recommendations by gender preference (BOTH) and exclude PNTS", async () => {
       ...
       const candidateOther = await seedTestUser({},{ gender: Gender.OTHER });
       const candidatePNTS = await seedTestUser({},{ gender: Gender.PREFER_NOT_TO_SAY });
    ...
    it("should filter primarily MALE recommendations for gender preference (MALE)", async () => {
        ...
        // Seed candidates
        const candidateMale1 = await seedTestUser({ email: "m1@e2e.com" },{ gender: Gender.MALE, lastActiveAt: new Date(Date.now() - 1000) });
        const candidateMale2 = await seedTestUser({ email: "m2@e2e.com" },{ gender: Gender.MALE, lastActiveAt: new Date(Date.now() - 2000) });
        const candidateFemale = await seedTestUser({ email: "f1@e2e.com" },{ gender: Gender.FEMALE, lastActiveAt: new Date(Date.now() - 3000) });
    ...
    it("should filter primarily FEMALE recommendations for gender preference (FEMALE)", async () => {
        ...
        // Seed candidates
        const candidateFemale1 = await seedTestUser({ email: "f1@e2e.com" },{ gender: Gender.FEMALE, lastActiveAt: new Date(Date.now() - 1000) });
        const candidateFemale2 = await seedTestUser({ email: "f2@e2e.com" },{ gender: Gender.FEMALE, lastActiveAt: new Date(Date.now() - 2000) });
        const candidateMale = await seedTestUser({ email: "m1@e2e.com" },{ gender: Gender.MALE, lastActiveAt: new Date(Date.now() - 3000) });
    ...
    it("should return 404 if the requesting user profile cannot be found", async () => {
        ...
        const noProfileToken = await generateTestJwt({ userId: userWithoutProfile.id });
