...
import { LessThan, IsNull, FindOptionsWhere, Or } from 'typeorm';
...
export class VenueMaintenanceService {
    ...
    async handleVenueStalenessCheck() {
        ...
        try {
            ...
            this.logger.error(`[${jobName}] Critical error during staleness check: ${error.message}`, error.stack);
        }
    }
    ...
    public async runStalenessCheckLogic() {
        ...
        this.logger.log(`[${jobName}] Finding venues not refreshed since ${cutoffDate.toISOString()} (Threshold: ${stalenessHours} hours).`);

        const staleVenues = await this.venueRepository.findStaleLocations(cutoffDate);
        ...
        this.logger.log(`[${jobName}] Found ${staleVenues.length} potentially stale venues.`);

        const uniqueGeohashPrefixes = new Set<string>();
        let decodeErrors = 0;
        for (const venue of staleVenues) {
            if (!venue.location) {
                this.logger.warn(`[${jobName}] Stale venue ${venue.id} missing location data. Skipping.`);
                continue;
            }
            try {
                ...
                if (match) {
                    ...
                    this.logger.warn(`[${jobName}] Could not parse WKT location for stale venue ${venue.id}: ${venue.location}`);
                    decodeErrors++;
                }
            } catch (error) {
                this.logger.error(`[${jobName}] Error encoding geohash for stale venue ${venue.id}: ${error.message}`, error.stack);
                decodeErrors++;
            }
        }
        
        const geohashList = Array.from(uniqueGeohashPrefixes);
        this.logger.log(`[${jobName}] Found ${geohashList.length} unique geohash areas to scan (Decode errors: ${decodeErrors}).`);

        let enqueuedCount = 0;
        for (const geohashPrefix of geohashList) {
            try {
                const { latitude, longitude } = ngeohash.decode(geohashPrefix);
                await this.venueScanProducerService.enqueueScanIfStale(latitude, longitude);
                enqueuedCount++;
            } catch (error) {
                this.logger.error(`[${jobName}] Error decoding/enqueuing geohash ${geohashPrefix}: ${error.message}`, error.stack);
            }
        }

        this.logger.log(`[${jobName}] Finished enqueuing ${enqueuedCount} scan jobs for stale areas.`);
