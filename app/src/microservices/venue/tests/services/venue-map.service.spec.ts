...
describe('VenueMapService', () => {
  ...
  beforeEach(async () => {
    ...
    const googleMapsServiceMock = {
      ...
      geocodeAddress: jest.fn().mockResolvedValue({ latitude: 40.7128, longitude: -74.0060 }),
    };
  ...
  describe('searchVenues', () => {
    it('should return local DB results and trigger background scan asynchronously', async () => {
      ...
      expect(venueRepository.search).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should throw error when latitude/longitude are not provided', async () => {
      const invalidDto = { ...mockSearchDto, latitude: undefined, longitude: undefined };
      
      await expect(service.searchVenues(invalidDto)).rejects.toThrow(BadRequestException);
    ...
    it('should handle errors and still return results', async () => {
      ...
      jest.spyOn(venueRepository, 'search').mockRejectedValueOnce(new Error('DB Error'));
  ...
  describe('getVenuesNearLocation', () => {
    it('should search DB and trigger background scan', async () => {
      ...
      expect(venueScanProducerService.enqueueScanIfStale).toHaveBeenCalledWith(lat, lon);
    });
    ...
    it('should handle errors and still trigger scans', async () => {
      ...
      const result = await service.getVenuesNearLocation(40.7128, -74.0060, 5);
