...
export class ScannedAreaRepository {
     ...
     */
    async upsertLastScanned(geohashPrefix: string, scannedAt: Date): Promise<ScannedArea> {
    ...
    async findLastScannedOrThrow(geohashPrefix: string): Promise<ScannedArea> {
        ...
        if (!scannedArea) {
            throw new NotFoundException(`Scanned area with prefix ${geohashPrefix} not found.`);
        }
        return scannedArea;
    }
} 