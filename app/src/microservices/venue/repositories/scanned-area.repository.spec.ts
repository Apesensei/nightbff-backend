...
describe("ScannedAreaRepository", () => {
  ...
  describe("upsertLastScanned", () => {
    it("should call create and save with correct data", async () => {
      ...
      const result = await repository.upsertLastScanned(
        geohashPrefix,
        scannedAt,
      );
  ...
  describe("findLastScannedOrThrow", () => {
    it("should return the entity if found", async () => {
      ...
      await expect(
        repository.findLastScannedOrThrow(geohashPrefix),
    ...
    it("should throw NotFoundException if not found", async () => {
      ...
      await expect(
        repository.findLastScannedOrThrow(geohashPrefix),
      ).rejects.toThrow(`Scanned area with prefix ${geohashPrefix} not found.`);
      expect(mockTypeOrmRepository.findOne!).toHaveBeenCalledWith({
        where: { geohashPrefix },
      });
    });
