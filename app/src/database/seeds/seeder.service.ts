...
import {
  ...
  UserRole,
  UserStatus,
} from "../../microservices/auth/entities/user.entity";
...
export class SeederService {
  ...
  private async seedAdminUser() {
    ...
    try {
      ...
      this.logger.error(
        "Error finding user. This may be expected if migrations just ran.",
      );
  ...
  private async seedTestUser() {
    ...
    try {
      existingTestUser = await this.userRepository.findOne({
        where: { email: testEmail },
      });
    } catch (e) {
      this.logger.error(
        "Error finding test user. This may be expected if migrations just ran.",
      );
      this.logger.error(e.message);
    }
