...
export const createGeneralRateLimit = (configService: ConfigService) =>
  rateLimit({
    ...
    max: parseInt(configService.get<string>("RATE_LIMIT_MAX_REQUESTS", "100")), // 100 requests per window
    message: {
...
export const createAuthRateLimit = (configService: ConfigService) =>
  rateLimit({
    ...
    max: parseInt(configService.get<string>("AUTH_RATE_LIMIT_MAX", "5")), // 5 auth attempts per 15 minutes
...
export const createUploadRateLimit = (configService: ConfigService) =>
  rateLimit({
    ...
    max: parseInt(configService.get<string>("UPLOAD_RATE_LIMIT_MAX", "10")), // 10 uploads per minute
    message: {
