...
export class InterestService {
  ...
  /**
   * NEW: Get event IDs associated with a specific interest ID
   */
  async getEventIdsByInterest(interestId: string): Promise<string[]> {
    // Input validation can happen here or rely on repository handling
    if (!interestId) {
      return [];
    }
    // Assumes interestRepository is injected as this.interestRepository
    return this.interestRepository.findEventIdsByInterest(interestId);
  }
